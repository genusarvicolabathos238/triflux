...
describe('deduplicateFindings — 중복 제거 + occurrences 카운트', () => {
  it('동일 description은 하나로 합치고 occurrences를 세야 한다', () => {
    const findings = [
      { description: 'Null pointer in auth module', file: 'auth.js', line: 10, severity: 'high' },
      { description: 'Null pointer in auth module', file: 'auth.js', line: 10, severity: 'high' },
      { description: 'Null pointer in auth module', file: 'auth.js', line: 10, severity: 'high' },
    ];
  ...
  it('유사도 80% 이상인 description은 중복으로 판정해야 한다', () => {
    ...
    const findings = [
      { description: 'Missing null check in auth handler for user login', severity: 'high' },
      { description: 'Missing null check in auth handler for user session', severity: 'medium' },
    ];
  ...
  it('유사도가 낮은 항목은 별도로 유지해야 한다', () => {
    const findings = [
      { description: 'SQL injection vulnerability in login endpoint', severity: 'critical' },
      { description: 'Memory leak in background worker process', severity: 'medium' },
    ];
    const result = deduplicateFindings(findings);
...
일반 텍스트 설명`;
  ...
  it('tokenEstimate가 500 이하여야 한다 (2000자 기준)', () => {
    ...
    const result = compressScoutReport(raw);
    assert.ok(result.tokenEstimate <= 500, `tokenEstimate ${result.tokenEstimate} exceeds 500`);
  });

  it('summary 길이가 2000자를 초과하지 않아야 한다', () => {
    const longReport = Array(100).fill('This is a very long sentence that should be truncated.').join('\n');
...
describe('weightedConsensus — 신뢰도 계산 정확성', () => {
  it('3명 중 3명이 같은 발견을 보고하면 confidence=1.0이어야 한다', () => {
    const reports = [
      { agentName: 'scout-1', findings: [{ description: 'Auth bypass vulnerability' }] },
      { agentName: 'scout-2', findings: [{ description: 'Auth bypass vulnerability' }] },
      { agentName: 'scout-3', findings: [{ description: 'Auth bypass vulnerability' }] },
    ];
    const result = weightedConsensus(reports);
    assert.equal(result.length, 1);
    assert.equal(result[0].confidence, 1.0);
    assert.deepEqual(result[0].reporters.sort(), ['scout-1', 'scout-2', 'scout-3']);
  });

  it('3명 중 1명만 보고하면 confidence~0.33이어야 한다', () => {
    const reports = [
      { agentName: 'scout-1', findings: [{ description: 'Unique finding only scout-1 found' }] },
      { agentName: 'scout-2', findings: [{ description: 'Different finding from scout-2' }] },
      { agentName: 'scout-3', findings: [{ description: 'Another different finding scout-3' }] },
    ];
    const result = weightedConsensus(reports);
  ...
  it('유사한 description을 가진 발견은 같은 그룹으로 묶어야 한다', () => {
    const reports = [
      { agentName: 'scout-1', findings: [{ description: 'Missing null check in auth handler' }] },
      { agentName: 'scout-2', findings: [{ description: 'Missing null check in auth handler function' }] },
    ];
    const result = weightedConsensus(reports);
    assert.equal(result.length, 1);
    assert.equal(result[0].confidence, 1.0);
...
describe('verifySlimWrapperRouteExecution — slopDetected 플래그', () => {
  it('동일 패턴 3회 이상 반복 시 slopDetected=true여야 한다', () => {
    const repeatedLine = 'This is a repeated output line that appears many times in the log';
