...
describe("pane.mjs", () => {
  ...
  it("buildCliCommand(codex, trustMode)는 exec + sandbox bypass를 포함해야 한다", () => {
    ...
    assert.ok(command.includes("--dangerously-bypass-approvals-and-sandbox"), `sandbox bypass 포함: ${command}`);
    assert.ok(command.includes("--skip-git-repo-check"), `skip-git 포함: ${command}`);
  });
