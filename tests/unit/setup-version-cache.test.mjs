...
describe('setup version cache', () => {
  it('writes the setup marker with the current package version after sync', () => {
    ...
    assert.equal(existsSync(markerPath), true, 'expected setup marker to be created');

    const marker = JSON.parse(readFileSync(markerPath, 'utf8'));
    assert.equal(marker.version, PACKAGE_VERSION);
    assert.equal(typeof marker.timestamp, 'number');
    assert.ok(marker.timestamp > 0, 'expected timestamp to be a positive number');
  });
  ...
  it('skips sync when the marker version already matches package.json', () => {
    ...
    writeFileSync(markerPath, JSON.stringify({ version: PACKAGE_VERSION, timestamp: 1234 }), 'utf8');

    const output = runSetup(tempHome);

    assert.match(output, new RegExp(`setup: skip \\(v${PACKAGE_VERSION} already synced\\)`));
    const marker = JSON.parse(readFileSync(markerPath, 'utf8'));
    assert.equal(marker.timestamp, 1234, 'skip path should not rewrite the marker');
  });
  ...
  it('--force ignores the marker and rewrites it after sync', () => {
    ...
    const markerPath = markerPathForHome(tempHome);
    mkdirSync(dirname(markerPath), { recursive: true });
    writeFileSync(markerPath, JSON.stringify({ version: PACKAGE_VERSION, timestamp: 1 }), 'utf8');
    ...
    const marker = JSON.parse(readFileSync(markerPath, 'utf8'));
    assert.equal(marker.version, PACKAGE_VERSION);
    assert.ok(marker.timestamp > 1, 'force path should rewrite the marker timestamp');
  });
});
