...
import { benchmarkStart, benchmarkEnd } from '../../hub/pipeline/index.mjs';
...
describe('token-benchmark hooks', () => {
  ...
  beforeEach(() => {
    ...
    origFiles = {
      snapshots: new Set(existsSync(SNAPSHOTS_DIR) ? readdirSync(SNAPSHOTS_DIR) : []),
  ...
  describe('benchmarkEnd()', () => {
    it('벤치마크 종료 시 diff를 계산하고 타임스탬프 파일을 저장한다', async () => {
      ...
      // 종료 + diff
      const diff = await benchmarkEnd(pre, post, { id: `__test_benchE_${Date.now()}` });
      ...
      // 타임스탬프 파일 확인
      const allDiffs = readdirSync(DIFFS_DIR).filter(f => f.endsWith('.json'));
